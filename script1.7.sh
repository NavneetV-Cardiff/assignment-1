...
# Function to create a CSV file from VCF
create_csv_file() {
    local vcf_file=$1
    local sample_name=$2

    # Create a combined spreadsheet from the VCF file
    echo
    echo "Creating a combined spreadsheet from the VCF file..."
    echo
    echo "CHROM,POS,REF,ALT,Sample1_genotype_info" > snps_with_genotype.csv
    grep -v "^##" "$vcf_file" | awk -F'\t' 'BEGIN {OFS=","} !/^#/ {split($10, genotype, ":"); print $1, $2, $4, $5, genotype[1]}' >> snps_with_genotype.csv
    echo "Done!"
}

# Call the function to create CSV files
create_csv_file "$SAMPLE.vcf" "$SAMPLE"

# Default values for variables
VARSCAN_JAR="/software/genomics/varscan/2.4.1/VarScan.jar"
PILEUP_FILE="$SAMPLE.pileup"
BASE_VCF="$SAMPLE.base.vcf"

# Function to rerun VarScan with different min-coverage settings and compare outputs
rerun_varscan_and_compare() {
    local sample=$1
    local pileup_file=$2
    local min_coverages=(5 10 15 20)
    local base_vcf="${sample}_base.vcf"

    # Run VarScan with default settings to create the base VCF file
    java -jar "$VARSCAN_JAR" mpileup2snp "$pileup_file" --output-vcf 1 > "$base_vcf"

    for min_coverage in "${min_coverages[@]}"; do
        local vcf_file="${sample}_mincov${min_coverage}.vcf"
        echo "Running VarScan with --min-coverage $min_coverage..."
        java -jar "$VARSCAN_JAR" mpileup2snp "$pileup_file" --output-vcf 1 --min-coverage "$min_coverage" > "$vcf_file"
        echo "Comparing $base_vcf with $vcf_file..."
        diff "$base_vcf" "$vcf_file" > "diff_mincov${min_coverage}.txt" || true
        echo "Differences saved to diff_mincov${min_coverage}.txt"
    done
}

# Call the function to rerun VarScan and compare outputs
rerun_varscan_and_compare "$SAMPLE" "$SAMPLE.pileup"

# DELETE the pileup file
echo
echo "Removing pileup file..."
#rm "$SAMPLE.pileup"  # Delete the pileup file to save space
